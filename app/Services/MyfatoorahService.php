...
class MyfatoorahService
{
    ...
    {
        // Fetch the latest Payment configuration from the database
        $paymentConfig = PaymentType::where('type','cash')->first();

        // Ensure that we have valid data, else fallback to default values
        if ($paymentConfig) {
            $this->myfatoorah_base_url = $paymentConfig->api_url ?? 'https://apitest.myfatoorah.com';
            $this->myfatoorah_api_key = $paymentConfig->api_key ?? '';
        } else {
            // Fallback to defaults if no config is available in the database
            $this->myfatoorah_base_url = 'https://apitest.myfatoorah.com';
            $this->myfatoorah_api_key = '';
        }

        // Set the callback URL from the config or fallback
        $this->myfatoorah_call_back_url = config('services.myfatoorah.call_back_url', 'https://dwafeerlms.com') . '/myfatoorah/callback';
        
        // Set payment URL based on the base URL
