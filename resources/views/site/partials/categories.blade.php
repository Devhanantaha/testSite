    ...
    <div class="swiper categories-swiper px-3">
      <div class="swiper-wrapper">
        ...
        <div class="swiper-slide">
          <a href="{{ route('workarea.show', $field->id) }}" class="d-block text-center text-white">
            <div class="category-card p-4 bg-white bg-opacity-10 rounded-4 shadow-sm">
              @if($field->images->isNotEmpty())

              <img src="{{ asset($field->images->first()->path) }}"
                class="img-fluid rounded mb-3"
                alt="{{ $field->title }}"
                style="height:100px;object-fit:contain;">
              @else
              <img src="{{ asset('images/default.jpg') }}" alt="default" class="img-fluid rounded">
              @endif
              <h5 class="fw-semibold text-white">{{ $field->description }}</h5>
            </div>
          </a>
        </div>
