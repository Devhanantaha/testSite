...
@section('content')
         <div id="content" class="site-content">
               <div class="container default-entry-header"><h1 class="entry-title simple-title">قائمة المقارنة</h1><div class="breadcrumbs circle-style" itemprop="breadcrumb"><a href="/ar">الرئيسية</a><span> قائمة المقارنة</span></div></div>
                              ...
                              <article id="post-9" class="post-9 page type-page status-publish hentry">
								 ...
								 @if($cl_content->count() > 0)                                 
                                    <table class="shop_table cart wishlist_table wishlist_view traditional responsive   " data-pagination="no" data-per-page="5" data-page="1" data-id="216" data-token="D48ONYS6RANT">
                                          <thead>
                                             <tr>
                                                <th class="product-name">الخاصية / المنتج</th>
                                                @foreach($cl_content as $wl )
                                                <th class="product-name">
                                                   <span class="nobr font-weight-bold">
                                                         {{ $wl->associatedModel->name2 }}  <a href="{{ route('comparelist.delete' , $wl->id) }}" class="remove remove_from_wishlist fa ml-3" title="Remove this product">&times;</a>		</span>
                                                </th>
                                                @endforeach
                                             </tr>
                                          </thead>
                                          <tbody class="wishlist-items-wrapper">
                                             ...
                                             <tr>
                                                   <td class="product-name">
                                                      ...
                                                      <td class="product-name">
                                                            @if($wl->quantity > 0)
                                                          <a id="addtocart" rel="nofollow" href="#4" data-price="{{  $wl->price }}" data-quantity="1" data-product_id="{{$wl->id}}"  class="button add_to_cart_button ajax_add_to_cart add_to_cart alt">اضافة للسلة</a>
                                                         @endif
                                                      </td>
                                                      @endforeach
                                             </tr>
