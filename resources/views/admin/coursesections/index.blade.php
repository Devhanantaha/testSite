...
<div class="page-header d-print-none">
  <div class="container-xl">
    <div class="row g-2 align-items-center">
      <div class="col">
        {{ Breadcrumbs::render('course-sections') }}
      ...
      <div class="col-auto ms-auto d-print-none">
        ...
        @canany(['course-sections-create'])
          <a href="{{ route($route.'.create') }}" class="btn btn-primary d-none d-sm-inline-block">
            ...
            @endcanany
            @canany(['course-sections-export'])
            
          <a href="{{ route('admin.bulk.export',['coursesections']) }}" class="btn btn-primary d-none d-sm-inline-block">
      ...
      <div class="col-12">
        <div class="card">
          ...
          <div class="table-responsive">
            <table class="table card-table table-vcenter text-nowrap" id="coursesectionsTable" style="padding:15px 5px 5px 5px;">
              <thead>
                <tr>
                  ...
                  <th> {{__('admin.coursesections.name')}}</th>
                  <th> {{__('admin.coursesections.courses_number')}}</th>
                  @canany(['course-sections-status'])
                  <th> {{__('admin.coursesections.status')}}</th>
                  @endcanany
               
                  <th>{{ __('admin.coursesections.field_photo') }}</th>
                  @canany(['course-sections-edit','course-sections-delete'])
                  <th>{{ __('admin.coursesections.actions') }}</th>
                  @endcanany
                </tr>
              </thead>
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <div class="form-check form-switch md-3">
                      ...
                      <input data-id="{{$row->id}}" data-type='App\Models\Section' class="form-check-input form-control toggole-class" type="checkbox" role="switch" id="flexSwitchCheckDefault" @if($row->active==1) checked="checked" @endif name="active">
                    </div>
                  </td>
                  @endcanany
...
<script>
  ...
  new DataTable('#coursesectionsTable', {
    "createdRow": function(row, data, dataIndex) {
      $('#coursesectionsTable').removeClass('dataTable');
      $('#coursesectionsTable').removeClass('datatable');
